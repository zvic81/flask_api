...
def is_mongo_run() -> int:
    client = pymongo.MongoClient(
        'localhost', 27017, serverSelectionTimeoutMS=3)
    try:
...
def select_logs_from_mongo(timestart: datetime, timeend: datetime, module: str = None) -> dict:
    ...
    res = collection.find(
        query, {'message': 1, '_id': 0, 'timestamp': 1, 'level': 1, 'module': 1})
    return list(res)


if __name__ == "__main__":
    r = select_logs_from_mongo(
        datetime(2023, 4, 30, 18, 00, 00), datetime(2023, 5, 2, 00, 00, 00), 'app')
    print(r)
